 ...
 * 1. Remove/Uninstall "ArduinoBLE" library if installed (this conflicts!)
 * 2. Only use ESP32's built-in BLE library
...
class ConfigCallbacks: public BLECharacteristicCallbacks {
    void onWrite(BLECharacteristic* pCharacteristic) {
      ...
      Serial.println("Config received: " + valueStr);
...
void initBLE() {
  ...
  // Create Status Characteristic (for device status)
...
void loop() {
  ...
  if (a.acceleration.x == 0 && a.acceleration.y == 0 && a.acceleration.z == 0 &&
    ...
    Serial.println("Zero readings detected - sensor may be stuck");
    delay(100);
    return;
  }
